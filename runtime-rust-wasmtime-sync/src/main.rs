...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("5 + 3 = {:?}", my_world.call_add_three(&mut store, 5));

    println!("ENV foo: {:?}", my_world.call_get_env(&mut store, "foo")?);
    println!("ENV bar: {:?}", my_world.call_get_env(&mut store, "bar")?);
    println!("ENV buz: {:?}", my_world.call_get_env(&mut store, "buz")?);
    println!(
        "ENV other: {:?}",
        my_world.call_get_env(&mut store, "other")?
    );
